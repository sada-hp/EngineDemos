...
void LoadMaterialSphereScene(GR::GrayEngine* Context)
{
	...
	Context->GetMainCamera().View.SetOffset({ -2.5, 50.0, 40.0 });
...
void UpdateResources(GR::GrayEngine* Context, double Delta)
{
	if (LoadedScene != SceneID)
	{
		if (SceneID == 0)
		{
			LoadMaterialSphereScene(Context);
...
void UI(GR::GrayEngine* Context, double Delta)
{
	...
	const char* Materials[3] = { "Material sphere", "R-M Spheres", "Model" };

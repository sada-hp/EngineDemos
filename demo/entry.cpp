...
{
	const double global_angle = glm::mod(Utils::GetTime(), 360.0);
...
inline void UpdateUI(Renderer& renderer, World& world)
{
	...
	if (LoadedScene == 0)
	{
		Components::RoughnessMultiplier& RM = world.GetComponent<Components::RoughnessMultiplier>(world.Registry.view<Entity>().front());
